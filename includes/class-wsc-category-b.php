...
class WSC_Category_B {
	...
	private function check_debug_display() {
		...
		if ( ! $debug_on ) {
			$status  = 'good';
			$message = '';
			$detail  = __( 'WP_DEBUG: 無効（本番環境として正常）', 'wp-security-checker' );
		} else {
			...
			if ( $display_on ) {
				$status  = 'recommended';
				$message = __( '本番でエラーが画面に表示される設定です。サーバーの構成やファイルの場所が攻撃者の手がかりになります。wp-config.phpでデバッグの画面表示をオフにしてください', 'wp-security-checker' );
				$detail  = __( 'WP_DEBUG: 有効／画面表示: あり', 'wp-security-checker' );
			} else {
				$status  = 'attention';
				$message = __( 'デバッグは有効ですが画面表示は抑止されています（ログのみ）。本番では不要ならWP_DEBUG自体の無効化を検討してください', 'wp-security-checker' );
				$detail  = __( 'WP_DEBUG: 有効／画面表示: なし（ログのみ）', 'wp-security-checker' );
			}
		}

		return array(
			'id'      => 'b1',
			'label'   => __( 'デバッグ表示', 'wp-security-checker' ),
			'status'  => $status,
			'message' => $message,
			'detail'  => $detail,
		);
	...
	private function check_file_editing() {
		...
		if ( $editing_disabled ) {
			$status  = 'good';
			$message = '';
			$detail  = __( 'DISALLOW_FILE_EDIT: 有効（管理画面からの編集を無効化済み）', 'wp-security-checker' );
		} else {
			$status  = 'recommended';
			$message = __( "管理者アカウントが乗っ取られるとコードをその場で書き換えられます。wp-config.phpに define('DISALLOW_FILE_EDIT', true); を追加してください", 'wp-security-checker' );
			$detail  = __( 'DISALLOW_FILE_EDIT: 未設定（管理画面からファイル編集が可能）', 'wp-security-checker' );
		}
	...
	private function check_admin_username() {
		...
		if ( empty( $found ) ) {
			$status  = 'good';
			$message = '';
			$detail  = __( '推測されやすい既定名の管理者は存在しません（良好）', 'wp-security-checker' );
		} else {
			$status  = 'attention';
			$message = __( '攻撃者が最初に試すのが既定の名前で、パスワード総当たりの標的になりやすいです。新しい名前の管理者を作って権限を移し、既定名のアカウントを削除してください（バックアップ後に実施）', 'wp-security-checker' );
			/* translators: %s: comma-separated list of default user names found */
			$detail  = sprintf( __( '既定名のユーザーが存在: %s', 'wp-security-checker' ), esc_html( implode( '、', $found ) ) );
		}
	...
	private function check_https() {
		...
		if ( $is_https ) {
			$status  = 'good';
			$message = '';
			$detail  = __( 'HTTPS: 有効（SSL証明書が適用されています）', 'wp-security-checker' );
		} else {
			$status  = 'recommended';
			$message = __( '通信が暗号化されず、ログイン情報などが途中で盗み見られる恐れがあります。SSL証明書を導入しhttpsに切り替えてください。多くのレンタルサーバーで無料の証明書が使えます', 'wp-security-checker' );
			$detail  = __( 'HTTPS: 無効（HTTP接続）', 'wp-security-checker' );
		}

		return array(
			'id'      => 'b4',
			'label'   => __( '常時HTTPS', 'wp-security-checker' ),
			'status'  => $status,
			'message' => $message,
			'detail'  => $detail,
		);
	...
	private function check_db_prefix() {
		...
		if ( 'wp_' === $prefix ) {
			$status  = 'attention';
			$message = __( '既定値のままだと一部の自動化された攻撃で狙いを定められやすくなります。新しくサイトを作る際は別の接頭辞にしてください。稼働中サイトは変更にリスクがあるため無理に変えないでください', 'wp-security-checker' );
		} else {
			$status  = 'good';
			$message = '';
		}
	...
	private function check_xmlrpc() {
		...
		if ( ! $xmlrpc_enabled ) {
			$status  = 'good';
			$message = '';
			$detail  = __( 'XML-RPC: 無効化されています', 'wp-security-checker' );
		} else {
			$status  = 'attention';
			$message = __( '使っていない場合、総当たり攻撃や踏み台攻撃の入り口になることがあります。外部連携アプリ（Jetpack等）や一部機能で使っていなければ無効化を検討してください。使っている場合はそのままで問題ありません', 'wp-security-checker' );
			$detail  = __( 'XML-RPC: 有効（xmlrpc.php が利用可能）', 'wp-security-checker' );
		}

		return array(
			'id'      => 'b6',
			'label'   => __( 'XML-RPC', 'wp-security-checker' ),
			'status'  => $status,
			'message' => $message,
			'detail'  => $detail,
		);
	...
	private function check_rest_user_enumeration() {
		...
		if ( ! $enumerable ) {
			$status  = 'good';
			$message = '';
			$detail  = __( '匿名でのユーザー名の列挙は抑止されています', 'wp-security-checker' );
		} else {
			$status  = 'attention';
			$message = __( 'ログインに使う名前が外部から集められ、総当たり攻撃の準備に使われます。ユーザー名の列挙を無効化する設定の追加を検討してください', 'wp-security-checker' );
			$detail  = __( '認証なしで /wp/v2/users からユーザー情報を取得できます', 'wp-security-checker' );
		}

		return array(
			'id'      => 'b7',
			'label'   => __( 'REST APIユーザー名列挙', 'wp-security-checker' ),
			'status'  => $status,
			'message' => $message,
			'detail'  => $detail,
		);
